...
%imgRoot=imread('C:\Users\USER\Desktop\dense_subgraph_saliency code\input\13_left.bmp');
...
for im_count=1:length(imnames);
    ...
    im_count;
    spnumber=800; %% superpixels number
    imname=[imgRoot imnames(im_count).name]; 
    input_im=imread(imname);
    %input_im=imread('C:\Users\USER\Desktop\dense_subgraph_saliency code\input\13_left.bmp');
    outname=[imgRoot imnames(im_count).name(1:end-5) '.bmp'];
...
%% generate superpixels
    ...
    %comm=['SLICSuperpixelSegmentation' ' ' ' ' int2str(20) ' '  ' ' supdir];
    system(comm);    
    spname=[supdir imnames(im_count).name(1:end-5)  '.dat'];
...
%% initial saliency map construction
   ...
   figure,imshow(I_gray,[]);
    ...
    gamma = 2;%gamma value 2 to 4 change
    ...
    gamma = 1;%gamma value change 4 to 3
